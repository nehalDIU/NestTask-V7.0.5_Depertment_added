...
export async function signupUser({ 
  ...
  try {
    ...
    // Create entry in the users table for extended profile
    ...
    if (profileError) {
      ...
      // Try to continue even if this fails - the auth trigger might handle it
    }
...
export function mapDbUserToUser(dbUser: SupabaseUser): User {
  ...
  let userRole: 'user' | 'admin' | 'super-admin' | 'section_admin' = 'user';
  ...
  } else if (dbUser.role === 'section_admin') {
    userRole = 'section_admin';
    console.log('Found section_admin user, setting role properly');
  }
