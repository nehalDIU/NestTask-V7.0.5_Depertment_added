  ...
  const loadTasks = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      if (isOffline) {
        ...
        console.log('[Debug] Offline mode: Loading tasks from IndexedDB');
        const offlineTasks = await getAllFromIndexedDB(STORES.TASKS);
        
        // Filter tasks for current user
        const userTasks = offlineTasks.filter((task: OfflineTask) => {
          if (userRole === 'section_admin' && userSectionId) {
            // For section admins, show tasks for their section or created by them
            return task.userId === userId || task.sectionId === userSectionId;
          } else if (userSectionId) {
            // For regular users with a section, show their tasks and section tasks
            return task.userId === userId || task.sectionId === userSectionId;
          } else {
            // Regular filtering for normal users without a section
        ...
        if (forceRefresh || cacheExpired) {
          ...
          console.log('[Debug] Using cached tasks - cache age:', Math.round(cacheAge / 1000), 'seconds');
          const offlineTasks = await getAllFromIndexedDB(STORES.TASKS);
          ...
          if (userTasks.length > 0) {
            ...
            console.log('[Debug] Cache empty, fetching fresh tasks');
            
            // Always pass sectionId for all user types
            const data = await fetchTasks(userId, userSectionId);
            
            console.log(`[Debug] Received ${data.length} tasks from server after empty cache`);
            setTasks(data);
    ...
    try {
      ...
      if (isOffline) {
        ...
        const offlineTask: OfflineTask = {
          ...
          sectionId // Add sectionId for section admins
        };
        ...
        // Create task online
        result = await createTask(userId, newTask, sectionId);
